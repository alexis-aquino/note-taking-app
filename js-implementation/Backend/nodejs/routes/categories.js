    ...
    try {
        const [rows] = await dbPool.query(
            `SELECT c.categoryId, c.categoryName,
             ...
             FROM categories c
             LEFT JOIN notes n ON n.categoryId = c.categoryId AND n.isTrash = 0 AND n.isArchived = 0
             WHERE c.userId = ?
...
categoriesRouter.get('/:categoryId/notes', async (req, res) => {
    ...
    try {
        const [rows] = await dbPool.query(
            `SELECT noteId, noteTitle, noteBody, isPinned, isFavorite, updatedAt
             FROM notes
             WHERE userId = ? AND categoryId = ? AND isTrash = 0 AND isArchived = 0
